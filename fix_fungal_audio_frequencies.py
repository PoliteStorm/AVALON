#!/usr/bin/env python3
"""
Fix Fungal Audio Frequencies - Make them audible while maintaining scientific accuracy
"""
...
class FungalAudioFrequencyFixer:
    ...
    def generate_fixed_audio_files(self):
        ...
        sf.write(f"{self.output_dir}/fungal_growth_rhythm_audible.wav", growth_audio, self.sample_rate)
        files_created.append("fungal_growth_rhythm_audible.wav")
        
        sf.write(f"{self.output_dir}/fungal_frequency_discrimination_audible.wav", freq_disc_audio, self.sample_rate)
        files_created.append("fungal_frequency_discrimination_audible.wav")
        
        sf.write(f"{self.output_dir}/fungal_harmonic_patterns_audible.wav", harmonic_audio, self.sample_rate)
        files_created.append("fungal_harmonic_patterns_audible.wav")
        
        sf.write(f"{self.output_dir}/fungal_integrated_communication_audible.wav", integrated_audio, self.sample_rate)
        ...
        for pattern_name, audio_data in linguistic_audio.items():
            ...
            sf.write(f"{self.output_dir}/{filename}", audio_data, self.sample_rate)
