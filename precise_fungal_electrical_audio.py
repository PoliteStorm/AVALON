#!/usr/bin/env python3
"""
Precise Fungal Electrical Audio Generator
...
class PreciseFungalElectricalAudio:
    ...
    def __init__(self):
        ...
        self.synth_params = {
            ...
            'filter_cutoff': 0.8        # Low-pass filter cutoff
        }
    ...
    def create_synth_waveform(self, frequency, amplitude, duration, waveform_type='saw'):
        ...
        filter_strength = self.synth_params['filter_cutoff']
        filtered = np.convolve(waveform, np.ones(int(len(waveform) * filter_strength)) / 
                              int(len(waveform) * filter_strength), mode='same')
    ...
    def create_precise_electrical_audio(self):
        ...
        for i in range(0, len(t), chunk_size):
            ...
            # Convert each voltage sample to audio
            for j, (time_val, voltage_val) in enumerate(zip(chunk_t, chunk_voltage)):
                # Map voltage to frequency and amplitude
                freq = self.voltage_to_audio_frequency(voltage_val)
                amp = self.voltage_to_audio_amplitude(voltage_val)
                
                # Create synth waveform for this moment
                waveform = self.create_synth_waveform(freq, amp, 1/self.sample_rate, 'saw')
                
                # Add to output
                if i + j < len(audio_output):
                    audio_output[i + j] = waveform[0]
