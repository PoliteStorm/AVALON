#!/usr/bin/env python3
"""
Improved Scaling Analysis: Square Root vs Linear Time Scaling
Integrated with Spike Detection and Complexity Analysis
...
- Integrated spike detection from the referenced paper
- Complexity analysis (entropy, fractal dimensions)
- Multiple sampling rates for variation testing
- Robust validation with multiple metrics
...
class ImprovedScalingAnalyzer:
    """
    Improved analyzer with spike detection integration and complexity analysis
    """
    ...
    def __init__(self):
        ...
        self.output_dir = Path("results/improved_scaling_analysis")
        ...
        print("=" * 70)
        print("Integrated with Spike Detection and Complexity Analysis")
        print("Based on: Dehshibi & Adamatzky (2021)")
        print("Wave Transform: W(k,τ) = ∫₀^∞ V(t) · ψ(√t/τ) · e^(-ik√t) dt")
        print("Features: No forced parameters, spike detection, complexity analysis")
        print("=" * 70)
    ...
    def detect_spikes_adaptive(self, signal_data: np.ndarray) -> Dict:
        """
        Detect spikes using adaptive methods (no forced parameters)
        Based on Dehshibi & Adamatzky (2021) methodology
        """
        print(f"🔍 Detecting spikes adaptively...")
        ...
        return {
            ...
            'mean_amplitude': np.mean(spike_amplitudes) if spike_amplitudes else 0,
            'mean_isi': np.mean(spike_isi) if spike_isi else 0,
            'isi_cv': np.std(spike_isi) / np.mean(spike_isi) if spike_isi and np.mean(spike_isi) > 0 else 0
        }
    
    def calculate_complexity_measures(self, signal_data: np.ndarray) -> Dict:
        """
        Calculate complexity measures as in Dehshibi & Adamatzky (2021)
        """
        ...
        apen = approximate_entropy(signal_data)
        ...
        sampen = sample_entropy(signal_data)
    ...
    def detect_adaptive_scales(self, signal_data: np.ndarray) -> List[float]:
        ...
        if len(scales) > 8:  # Reduced from 10 for efficiency
            scales = sorted(list(set(scales)))[:8]
        else:
  ...
  - Complexity Analysis: ✅
    ...
    def process_single_file_multiple_rates(self, csv_file: str) -> Dict:
        ...
        for sampling_rate in self.adamatzky_settings['sampling_rates']:
            ...
            complexity_data = self.calculate_complexity_measures(signal_data)
...
def main():
    ...
    analyzer = ImprovedScalingAnalyzer()
    ...
    if results and 'error' not in results:
        ...
        print("📁 Results saved in results/improved_scaling_analysis/")
