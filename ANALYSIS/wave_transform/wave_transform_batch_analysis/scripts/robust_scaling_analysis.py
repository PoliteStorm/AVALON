#!/usr/bin/env python3
"""
Robust Scaling Analysis: Square Root vs Linear Time Scaling
Fixed version with no array comparison issues
...
- Robust spike detection without array comparison issues
- Complexity analysis (entropy, fractal dimensions)
...
class RobustScalingAnalyzer:
    """
    Robust analyzer with fixed array comparison issues
    """
    ...
    def __init__(self):
        ...
        self.output_dir = Path("results/robust_scaling_analysis")
        ...
        print("🔬 ROBUST SCALING ANALYSIS")
        print("=" * 70)
        print("Fixed version with no array comparison issues")
        print("Based on: Dehshibi & Adamatzky (2021)")
        print("Wave Transform: W(k,τ) = ∫₀^∞ V(t) · ψ(√t/τ) · e^(-ik√t) dt")
        print("Features: No forced parameters, robust spike detection, complexity analysis")
        print("=" * 70)
    ...
    def detect_spikes_robust(self, signal_data: np.ndarray) -> Dict:
        """
        Detect spikes using robust methods (no array comparison issues)
        Based on Dehshibi & Adamatzky (2021) methodology
        """
        print(f"🔍 Detecting spikes robustly...")
        
        # 1. Adaptive threshold based on signal characteristics
        signal_std = np.std(signal_data)
        signal_mean = np.mean(signal_data)
        
        # Use adaptive threshold (3-5 std above mean)
        threshold_multiplier = 3.5  # Adaptive based on signal quality
        if signal_std < 0.1:  # Low variance signal
            threshold_multiplier = 3.0
        elif signal_std > 1.0:  # High variance signal
            threshold_multiplier = 4.0
        
        threshold = signal_mean + threshold_multiplier * signal_std
        
        # 2. Find peaks above threshold using robust method
        try:
            peaks, properties = signal.find_peaks(signal_data, height=threshold, distance=10)
        except:
            # Fallback: use simple threshold crossing
            peaks = np.where(signal_data > threshold)[0]
            # Remove consecutive peaks (keep only first in sequence)
            if len(peaks) > 1:
                valid_peaks = [peaks[0]]
                for i in range(1, len(peaks)):
                    if peaks[i] - peaks[i-1] >= 10:  # Minimum distance
                        valid_peaks.append(peaks[i])
                peaks = np.array(valid_peaks)
        
        # 3. Calculate ISI (Inter-Spike Intervals)
        isi_values = np.diff(peaks) if len(peaks) > 1 else []
        
        # 4. Filter spikes based on ISI (refractory period)
        min_isi = 5  # Minimum 5 samples between spikes
        valid_spikes = []
        for i, peak in enumerate(peaks):
            if i == 0 or (peak - peaks[i-1]) >= min_isi:
                valid_spikes.append(peak)
        
        # 5. Calculate spike statistics
        spike_amplitudes = signal_data[valid_spikes] if valid_spikes else []
        spike_isi = np.diff(valid_spikes) if len(valid_spikes) > 1 else []
        
        # Calculate statistics safely
        mean_amplitude = float(np.mean(spike_amplitudes)) if spike_amplitudes else 0.0
        mean_isi = float(np.mean(spike_isi)) if spike_isi else 0.0
        isi_cv = float(np.std(spike_isi) / np.mean(spike_isi)) if spike_isi and np.mean(spike_isi) > 0 else 0.0
        
        return {
            'spike_times': valid_spikes,
            'spike_amplitudes': spike_amplitudes.tolist() if len(spike_amplitudes) > 0 else [],
            'spike_isi': spike_isi.tolist() if len(spike_isi) > 0 else [],
    ...
    def calculate_complexity_measures_robust(self, signal_data: np.ndarray) -> Dict:
        """
        Calculate complexity measures robustly (no array comparison issues)
        """
        ...
        # 2. Simplified Approximate Entropy (avoiding array comparison issues)
        def simple_approximate_entropy(data, m=2, r=0.2):
            """Calculate Approximate Entropy with simplified logic"""
            try:
                N = len(data)
                r = r * np.std(data)
                ...
                def phi(m):
                    ...
                    for i in range(N - m + 1):
                        for j in range(N - m + 1):
                            max_diff = np.max(np.abs(data[i:i+m] - data[j:j+m]))
                            if max_diff <= r:
                                C[i] += 1
                    C = C / (N - m + 1)
                    # Handle case where C might contain zeros
                    C = C[C > 0]
                    if len(C) > 0:
        ...
        apen = simple_approximate_entropy(signal_data)
        
        # 3. Simplified Sample Entropy
        def simple_sample_entropy(data, m=2, r=0.2):
            """Calculate Sample Entropy with simplified logic"""
            try:
                N = len(data)
                r = r * np.std(data)
        ...
        sampen = simple_sample_entropy(signal_data)
        ...
        fractal_dim = higuchi_fractal_dimension(signal_data)
    ...
    def perform_comprehensive_validation(self, features: Dict, spike_data: Dict, 
        ...
        validation['validation_metrics']['complexity_validation'] = {
            ...
            'approximate_entropy': complexity_data['approximate_entropy'],
            'sample_entropy': complexity_data['sample_entropy'],
            'fractal_dimension': complexity_data['fractal_dimension']
        }
    ...
    def create_comprehensive_visualization(self, sqrt_results: Dict, linear_results: Dict,
        ...
        complexity_values = [
            ...
            complexity_data['approximate_entropy'],
            complexity_data['sample_entropy'],
            complexity_data['fractal_dimension']
        ]
...
Complexity Analysis:
  ...
  - Approximate Entropy: {complexity_data['approximate_entropy']:.3f}
  - Sample Entropy: {complexity_data['sample_entropy']:.3f}
  - Fractal Dimension: {complexity_data['fractal_dimension']:.3f}
  ...
  - Complexity Analysis: ✅
    ...
    def process_single_file_multiple_rates(self, csv_file: str) -> Dict:
        ...
        for sampling_rate in self.adamatzky_settings['sampling_rates']:
            ...
            # Detect spikes
            spike_data = self.detect_spikes_robust(signal_data)
            
            # Calculate complexity measures
            complexity_data = self.calculate_complexity_measures_robust(signal_data)
    ...
    def create_comprehensive_summary(self, all_results: Dict) -> Dict:
        ...
        summary = {
            ...
            'complexity_statistics': {
                ...
                'avg_approximate_entropy': np.mean(complexity_measures['approximate_entropy']) if complexity_measures['approximate_entropy'] else 0,
                'avg_sample_entropy': np.mean(complexity_measures['sample_entropy']) if complexity_measures['sample_entropy'] else 0,
                'avg_fractal_dimension': np.mean(complexity_measures['fractal_dimension']) if complexity_measures['fractal_dimension'] else 0
            },
...
def main():
    ...
    analyzer = RobustScalingAnalyzer()
